...
func (q *queue[T]) DequeueLeft() (T, error) {
	...
	// Get the prev node after the current tail node, which will become the new tail of the queue.
	...
	// Set the prev node after the current tail node to be the new tail of the queue.
